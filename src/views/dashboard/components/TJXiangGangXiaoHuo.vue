...
export default {
  ...
  data: () => ({
    GuoNei: false, // 国内品牌(比如小米产品)
    XiangGang: false, // 香港交货
  ...
  watch: {
    GuoNei: function(newVal) {
      ...
      if (newVal === true) {
        this.newTotalPrice += CountWeight * 5
        msg.push(`国内品牌 5 元/KG: ${CountWeight * 5}`)
      } else {
        this.newTotalPrice -= CountWeight * 5
        msg.splice(msg.indexOf(`国内品牌 5 元/KG: ${CountWeight * 5}`), 1)
      }
      this.item.showInfo.totalPrice = this.newTotalPrice.toFixed(2)
      this.item.showInfo.msg = msg
    },
    XiangGang: function(newVal) {
      // 香港交货 300
      this.newTotalPrice = parseFloat(this.item.showInfo.totalPrice)
      const msg = this.item.showInfo.msg
      if (newVal === true) {
        this.newTotalPrice += 300
        msg.push(`香港交货 : 300`)
      } else {
        this.newTotalPrice -= 300
        msg.splice(msg.indexOf(`香港交货 : 300`), 1)
      }
      this.item.showInfo.totalPrice = this.newTotalPrice.toFixed(2)
      this.item.showInfo.msg = msg
    },
    PianYuan: function(newVal) {
      // 偏远附加费 4.5 元/KG, 最低收费 230 元
      this.newTotalPrice = parseFloat(this.item.showInfo.totalPrice)
      const CountWeight = Number(this.item.showInfo.CountWeight) / 1000
      const msg = this.item.showInfo.msg
      if (newVal === true) {
        this.newTotalPrice += CountWeight * 4.5 >= 230 ? CountWeight * 4.5 : 230
        msg.push(`偏远附加费 4.5 元/KG(最低230): ${CountWeight * 4.5 >= 230 ? CountWeight * 4.5 : 230}`)
      } else {
        this.newTotalPrice -= CountWeight * 4.5 >= 230 ? CountWeight * 4.5 : 230
        msg.splice(msg.indexOf(`偏远附加费 4.5 元/KG(最低230): ${CountWeight * 4.5 >= 230 ? CountWeight * 4.5 : 230}`), 1)
      }
      this.item.showInfo.totalPrice = this.newTotalPrice.toFixed(2)
      this.item.showInfo.msg = msg
    },
  ...
  methods: {
    ...
    forUPSPrice(value, volume) {
      ...
      if (this.lwh_arr.length === 3 && (this.lwh_arr[0] >= 118 || this.lwh_arr[1] >= 74)) {
        ChaoZhong = 108
        msg.push('最长边 >= 118 CM / 第二边 >= 74 CM: 108元')
      }
      // 超重附加费：单件结算重超30KG（包含30KG）
      if (CountWeight / 1000 >= 31000) {
        ChaoZhong = 108
        msg.push('单件计费重大于等于31kg: 222元')
      }
      if (volume > 290) {
        ChaoZhong += 527
        msg.push('围长 > 290 CM: 527元')
      }
      ...
      this.$emit('countPrice', { totalPrice, isShow: true, msg, CountWeight }, this.index, 'TJXiangGangXiaoHuo.vue')
    }
  }
}
