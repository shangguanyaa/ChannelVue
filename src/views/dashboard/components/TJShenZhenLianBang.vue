...
export default {
  ...
  watch: {
    ...
    PianYuan: function(newVal) {
      ...
      const msg = this.item.showInfo.msg
      if (newVal === true) {
        this.newTotalPrice += CountWeight * 4.6 * getOil() >= 218 * getOil() ? parseFloat((CountWeight * 4.6 * getOil()).toFixed(2)) : parseFloat((218 * getOil()).toFixed(2))
        msg.push(`偏远收费 4.6 / KG * 燃油 ${getOil()} ,最低 218 元 * 燃油 ${getOil()} /票: ${CountWeight * 4.6 * getOil() >= 218 * getOil() ? parseFloat((CountWeight * 4.6 * getOil()).toFixed(2)) : parseFloat((218 * getOil()).toFixed(2))}`)
      } else {
        this.newTotalPrice -= CountWeight * 4.6 * getOil() >= 218 * getOil() ? parseFloat((CountWeight * 4.6 * getOil()).toFixed(2)) : parseFloat((218 * getOil()).toFixed(2))
        msg.splice(msg.indexOf(`偏远收费 4.6 / KG * 燃油 ${getOil()} ,最低 218 元 * 燃油 ${getOil()} /票: ${CountWeight * 4.6 * getOil() >= 218 * getOil() ? parseFloat((CountWeight * 4.6 * getOil()).toFixed(2)) : parseFloat((218 * getOil()).toFixed(2))}`), 1)
      }
      this.item.showInfo.totalPrice = this.newTotalPrice.toFixed(2)
      this.item.showInfo.msg = msg
    },
  ...
  methods: {
    ...
    forUPSPrice(value, volume) {
      ...
      if (this.lwh_arr.length === 3 && (this.lwh_arr[0] >= 118 || this.lwh_arr[1] >= 74)) {
        ...
        msg.push(`(超重超长不叠加) 最长边 >= 118 CM / 第二边长 >= 74 CM: 108 * 燃油 ${getOil()} = ${ChaoZhong} 元`)
      }
      // 或围长超264厘米
      if (volume > 264) {
        ChaoZhong = parseFloat((108 * getOil()).toFixed(2))
        msg.push(`(超重超长不叠加) 围长 > 264 CM: 108 * 燃油 ${getOil()} = ${ChaoZhong} 元`)
      }
      // 超重附加费：单件结算重超30KG（包含30KG）
      if (CountWeight / 1000 >= 30) {
        ChaoZhong = parseFloat((108 * getOil()).toFixed(2))
        msg.push(`(超重超长不叠加) 单件结算重超30KG: 108 * 燃油 ${getOil()} = ${ChaoZhong} 元`)
      }
      // 最长边超240cm或围长超325cm但不超过最长边265cm，周边长（1长+2宽+2高）≤328cm的情况可以发，需要加附加费516元/票*燃油
      if ((this.lwh_arr.length === 3 && this.lwh_arr[0] > 240) || volume > 325) {
        ChaoZhong += parseFloat((516 * getOil()).toFixed(2))
        msg.push(`最长边超240cm或围长超325cm: 516 * 燃油 ${getOil()} = ${parseFloat((516 * getOil()).toFixed(2))} 元`)
      }
